...
/**
 * Tworzy zamówienie sklepowe i przypisuje dostawę do {@code employee@example.com}
 * (pełna ścieżka notifierów: create → IN_DELIVERY → assign / FCM kuriera).
 */
@Service
...
public class NotificationTestHookService {
    ...
    public NotificationTestHookResponse createOrderAndAssignToSeedEmployee() {
        ...
        User employee = userRepository.findByEmail(EMPLOYEE_EMAIL)
                .orElseThrow(() -> new BusinessException(
                        HttpStatus.PRECONDITION_FAILED,
                        "Brak konta demo " + EMPLOYEE_EMAIL));

        long productId = productStockRepository.findAll().stream()
        ...
        OrderResponse created = orderService.create(customer.getId(), new CreateOrderRequest(
                clientOrderNumber,
        ...
        long deliveryId = deliveryRepository.findByOrderId(inDelivery.id())
                .orElseThrow(() -> new BusinessException(HttpStatus.INTERNAL_SERVER_ERROR, "Brak Delivery po IN_DELIVERY"))
                .getId();
        ...
        return new NotificationTestHookResponse(
                ...
                employee.getId(),
                employee.getEmail(),
                "FCM kuriera: zaloguj się jako employee@, POST /api/devices/fcm z tokenem, potem wywołaj ten hook."
        );
    }
