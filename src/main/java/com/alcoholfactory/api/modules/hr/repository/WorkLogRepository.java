...
public interface WorkLogRepository extends JpaRepository<WorkLog, Long> {
    ...
    @Query("SELECT w FROM WorkLog w WHERE w.employee.id = :empId AND w.clockOutAt IS NULL ORDER BY w.clockInAt DESC")
    ...
    @Query("SELECT w FROM WorkLog w JOIN FETCH w.employee WHERE w.clockInAt >= :from AND w.clockInAt <= :to AND w.clockOutAt IS NOT NULL")
