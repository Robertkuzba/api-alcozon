...
public class WorkLogService {
  ...
  public WorkLogResponse clockIn(Long employeeId, String notes) {
    workLogRepository
        .findFirstOpenSession(employeeId)
        .ifPresent(
            w -> {
    ...
    WorkLog w = WorkLog.builder().employee(emp).clockInAt(Instant.now()).notes(notes).build();
  ...
  public List<WorkLogResponse> my(Long employeeId, Instant from, Instant to) {
    return workLogRepository
        .findByEmployeeIdAndClockInAtBetweenOrderByClockInAtDesc(employeeId, from, to)
        .stream()
        .map(this::toResponse)
        .toList();
  }
