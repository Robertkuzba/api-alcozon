...
public class WarehouseService {
  ...
  public ReplenishmentOrderResponse create(Long managerId, CreateReplenishmentRequest req) {
    ...
    for (ReplenishmentLineRequest line : req.lines()) {
      validateLine(line);
      ReplenishmentLine rl =
          ReplenishmentLine.builder()
              .replenishment(order)
    ...
    return replenishmentOrderRepository
        .findWithDetailsById(saved.getId())
        .map(this::toResponse)
        .orElseThrow(
            () ->
                new BusinessException(
                    HttpStatus.INTERNAL_SERVER_ERROR, "Replenishment order not found after save"));
  }
  ...
  private void validateLine(ReplenishmentLineRequest line) {
    ...
    if (p == r) {
      throw new BusinessException(
          HttpStatus.BAD_REQUEST, "Each line needs exactly one of productId or rawMaterialId");
    }
  }
  ...
  public ReplenishmentOrderResponse applyStatus(Long orderId, PatchReplenishmentStatusRequest req) {
    ...
    ReplenishmentOrder order =
        replenishmentOrderRepository
            ...
            .orElseThrow(
                () -> new BusinessException(HttpStatus.NOT_FOUND, "Replenishment order not found"));
    ...
    for (ReplenishmentLine line : order.getLines()) {
      if (line.getProduct() != null) {
        inventoryService.patchProductStock(
            line.getProduct().getId(), line.getQuantityDelta().intValue());
      } else if (line.getRawMaterial() != null) {
        inventoryService.patchRawMaterial(line.getRawMaterial().getId(), line.getQuantityDelta());
      }
    }
  }

  private ReplenishmentOrderResponse toResponse(ReplenishmentOrder order) {
    List<ReplenishmentLineResponse> lineDtos =
