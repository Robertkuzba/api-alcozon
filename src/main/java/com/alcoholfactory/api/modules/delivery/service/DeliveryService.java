...
public class DeliveryService {
  ...
  public List<DeliveryResponse> my(Long courierId) {
    return deliveryRepository.findByCourierIdFetched(courierId).stream()
        .map(this::toResponse)
        .toList();
  }
  ...
  private DeliveryResponse toResponse(Delivery d) {
    ...
    if (clientOrderNumber == null || clientOrderNumber.isBlank()) {
      if (custom) {
        ...
        clientOrderNumber =
            co.getClientOrderNumber() != null ? co.getClientOrderNumber() : "CUSTOM-" + co.getId();
      } else {
