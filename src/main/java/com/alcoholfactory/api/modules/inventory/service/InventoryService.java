...
public class InventoryService {
    ...
    public InventoryOverviewResponse overview() {
        var products = productStockRepository.findAllWithProduct().stream()
                .map(ps -> new InventoryProductRow(
                        ps.getProduct().getId(),
                ...
                .toList();
        var raw = rawMaterialRepository.findAll().stream()
    ...
    public InventoryProductRow patchProductStock(Long productId, int delta) {
        ...
        ProductStock ps = productStockRepository.findById(productId)
    ...
    public InventoryRawRow patchRawMaterial(Long id, BigDecimal delta) {
        RawMaterial r = rawMaterialRepository.findById(id)
                .orElseThrow(() -> new BusinessException(HttpStatus.NOT_FOUND, "Raw material not found"));
