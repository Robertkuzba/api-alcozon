...
public class EmployeePasswordResetMailService {
  ...
  public void sendNewPassword(String toEmail, String plainPassword, UserRole role) {
    ...
    String loginHint =
        manager
            ? "Zaloguj sie w aplikacji Desktop (staff/login) i przy pierwszym logowaniu potwierdz"
                + " urzadzenie kodem 2FA z e-maila."
            : "Zaloguj sie w aplikacji mobilnej (staff/login) i przy pierwszym logowaniu potwierdz"
                + " urzadzenie kodem 2FA z e-maila.";
    String body =
        """
        ...
        """
            .formatted(manager ? "konto managera" : "konto pracownika", plainPassword, loginHint);
    ...
    try {
      ...
      log.error(
          "Staff password reset email failed role={} to={}. Cause: {}",
          role,
          toEmail,
          ex.getMessage());
      log.warn(
