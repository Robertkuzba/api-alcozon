...
public class AuthService {
  ...
  public TokenResponse login(LoginRequest req) {
    User user =
        userRepository
            .findByEmail(req.email().toLowerCase().trim())
    ...
    if (STAFF_ROLES.contains(user.getRole())) {
      throw new BusinessException(
          HttpStatus.BAD_REQUEST, "Staff must use POST /api/auth/staff/login with deviceId");
    }
    return issueTokens(user);
  }
  ...
  public TokenResponse refresh(RefreshRequest req) {
    ...
    RefreshToken stored =
        refreshTokenRepository
            ...
            .orElseThrow(
                () -> new BusinessException(HttpStatus.UNAUTHORIZED, "Invalid refresh token"));
  ...
  public TokenResponse confirmAgeAndReissue(AppUserDetails principal) {
    ...
    if (user.getRole() == UserRole.GUEST) {
      ...
      throw new BusinessException(
          HttpStatus.BAD_REQUEST, "Age confirmation is only for guests and customers");
    }
    userRepository.save(user);
    return issueTokens(user);
  }
  ...
  private TokenResponse issueTokens(User user) {
    ...
    refreshTokenRepository.save(
        RefreshToken.builder().user(user).tokenHash(hash).expiresAt(exp).revoked(false).build());
