...
public class StaffAuthService {
  ...
  public TokenResponse verifyDevice(VerifyDeviceRequest req) {
    ...
    String deviceId = normalizeDeviceId(req.deviceId());
    DeviceVerificationChallenge challenge =
        challengeRepository
            .findByIdAndConsumedAtIsNull(req.challengeId())
            .orElseThrow(
                () ->
                    new BusinessException(HttpStatus.UNAUTHORIZED, "Invalid or expired challenge"));
  ...
  private User authenticateStaff(String email, String password) {
    ...
    if (!STAFF_ROLES.contains(user.getRole())) {
      throw new BusinessException(
          HttpStatus.FORBIDDEN, "Staff login only for EMPLOYEE and MANAGER");
    }
  ...
  private StaffLoginResponse startVerificationChallenge(User user, String deviceId) {
    ...
    DeviceVerificationChallenge challenge =
        DeviceVerificationChallenge.builder()
            .user(user)
            .deviceId(deviceId)
        ...
        .ifPresentOrElse(
            ...
            () ->
                trustedDeviceRepository.save(
                    TrustedDevice.builder().user(user).deviceId(deviceId).lastUsedAt(now).build()));
  }
