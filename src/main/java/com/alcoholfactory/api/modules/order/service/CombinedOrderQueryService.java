...
public class CombinedOrderQueryService {
    ...
    public StaffCombinedOrdersResponse listForStaff(Pageable pageable) {
        Page<OrderResponse> shop = orderRepository.findAll(pageable)
                .map(o -> orderRepository.findDetailById(o.getId()).orElse(o))
                .map(orderService::toResponse);
        List<CustomOrderResponse> custom = customOrderRepository.findAllWithUsers().stream()
    ...
    public CombinedOrdersResponse forCourier(Long courierUserId, Long actorUserId, boolean manager) {
        ...
        List<OrderResponse> shop = orderRepository.findInDeliveryAssignedToCourier(
                        courierUserId,
                ...
                .map(orderService::toResponse)
                .toList();
        List<CustomOrderResponse> custom = customOrderRepository.findInDeliveryAssignedToCourier(
                        courierUserId,
                        OrderStatus.IN_DELIVERY
                ).stream()
                .map(customOrderService::toResponse)
