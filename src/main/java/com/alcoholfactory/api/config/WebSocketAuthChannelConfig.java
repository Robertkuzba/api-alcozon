...
public class WebSocketAuthChannelConfig implements WebSocketMessageBrokerConfigurer {
  ...
  private void authenticateConnect(Message<?> message, StompHeaderAccessor accessor) {
    ...
    if (headers == null || headers.isEmpty()) {
      throw new MessageDeliveryException(
          message, new BadCredentialsException("Missing Authorization for STOMP CONNECT"));
    }
    String raw = headers.getFirst();
    if (raw == null || !raw.startsWith("Bearer ")) {
      throw new MessageDeliveryException(
          message, new BadCredentialsException("STOMP CONNECT requires Bearer token"));
    }
    String token = raw.substring(7).trim();
    if (token.isEmpty()) {
      throw new MessageDeliveryException(
          message, new BadCredentialsException("Empty bearer token"));
    }
    try {
      ...
      if (email == null) {
        throw new MessageDeliveryException(
            message, new BadCredentialsException("Token without email claim"));
      }
      var authorities =
          role == null
      ...
      throw new MessageDeliveryException(
          message, new BadCredentialsException("Invalid or expired STOMP token", e));
    }
  }
  ...
  private void authorizeSubscribe(Message<?> message, StompHeaderAccessor accessor) {
    if (!(accessor.getUser() instanceof Authentication auth)) {
      throw new MessageDeliveryException(
          message, new BadCredentialsException("Unauthenticated STOMP subscription"));
    }
