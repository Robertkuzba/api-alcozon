...
public class CustomOrderRealtimeNotifier {
    ...
    public void onCustomOrderCreated(CustomOrder order) {
        OrderRealtimeEvent event = OrderRealtimeEvent.of(
    ...
    public void onCustomOrderStatusChanged(CustomOrder order, OrderStatus newStatus) {
        ...
        if (newStatus == OrderStatus.IN_DELIVERY) {
            Long deliveryId = deliveryRepository.findByCustomOrderId(order.getId())
                    .map(Delivery::getId)
                    .orElse(null);
            OrderRealtimeEvent dispatch = new OrderRealtimeEvent(
    ...
    public void onCustomDeliveryAssigned(Delivery delivery) {
        ...
        if (courier != null && StringUtils.hasText(courier.getEmail())) {
            messagingTemplate.convertAndSendToUser(
                    courier.getEmail(),
                    OrderRealtimeDestinations.COURIER_QUEUE,
                    event
            );
            fcmPush.notifyUser(
                    courier.getId(),
                    "Nowa dostawa (własne)",
                    label(order) + " — adres w aplikacji",
                    event
            );
        }
    }
    ...
    private void toCustomer(CustomOrder order, OrderRealtimeEvent event) {
        ...
        messagingTemplate.convertAndSendToUser(
                email,
                OrderRealtimeDestinations.CUSTOMER_QUEUE,
                event
        );
    }
